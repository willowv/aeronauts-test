...
function ScenarioReportVis(
  ...
  return (
    <Box mx="2">
      ...
      <p>{`${report.avgActionCount.mean.toFixed(
        2
      )} (sd ${report.avgActionCount.sd.toFixed(2)})`}</p>
      <p>{`${report.avgEnemyActionCount.mean.toFixed(
        2
      )} (sd ${report.avgEnemyActionCount.sd.toFixed(2)})`}</p>
      <p>{`${report.avgRoundCount.mean.toFixed(
        2
      )} (sd ${report.avgRoundCount.sd.toFixed(2)})`}</p>
    </Box>
  );
}
...
export class StatsVis extends React.Component<StatsVisProps> {
  render() {
    ...
    if (this.props.reports.length > 0) {
      let reportsToDisplay = this.props.reports
        .slice(Math.max(this.props.reports.length - 10, 0))
        .reverse();
